...
const handleWildCard = function(env, path) {
  switch(path) {
    case '~': return env.home;
    case '-': return env.oldPwd;
    case '*': return fs.readdirSync(env.pwd).join(' ');
  }
...
const cd = function(env, path) {
  const absPath = getAbsPath(env, handleWildCard(env, path));
...
const echo = function(env, text) {
  return updateState(env, console.log(handleWildCard(env, text)));
};
